...
struct EthcallNode * build_call_tree(struct EthcallNode *root, t_rpcResponse * rpc_getlogs,struct  arguments *arg)
{
    ...
    for(size_t i =0; i < logs_count; i++) {               
        ...
        /* ---   eth_getCode --- 
        // blockNumber requires all transactional data in the past 
        // most netwrok don't have it
        json_object *blocknumber  = json_object_new_string("latest"); 
        t_rpcResponse rpc_getcode = {0};

        
        if(get_bytecode(json_object_get_string(address)
                    , json_object_get_string(blocknumber)
                    , arg
                    , &rpc_getcode)!=0)
        {
            // eth_getCode failed
            fprintf(stderr, "Error: eth_getCode failed for address [%s] - Skipping ", json_object_get_string(address));
            continue;
        }
        
        free_rpc_response(&rpc_getcode);
        */
    }
